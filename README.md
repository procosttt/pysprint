...
Мобильный тренажёр коротких задач по Python. Этап 2: запуск кода через Pyodide в отдельном Web Worker, без проверки решений по тестам.

## Что умеет сейчас

- Список из трёх задач, экран условия и редактор на `textarea`
- Панель быстрых Python-клавиш и черновики в `localStorage`
- Кнопка **Запустить** выполняет текущий код с вводом первого открытого примера
...
- Кнопка **Проверить** пока отключена (этап 3)
